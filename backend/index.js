...
/* =====================================================
   POST — RESYNC INCONSISTENCIAS (TEMPORAL SIN AUTH)
   BORRAR DESPUÉS DE USAR
===================================================== */
app.post('/temp-fix-inconsistencies-xyz789', async (req, res) => {
  try {
    const storeId = process.env.TIENDANUBE_STORE_ID;
    ...
    const orders = ordersRes.rows;
    console.log(`🔄 TEMP FIX: Resync de ${orders.length} pedidos con inconsistencias`);

    if (orders.length === 0) {
      return res.json({ message: 'No hay pedidos con inconsistencias pendientes', resynced: 0 });
    }
    ...
    for (const order of orders) {
      try {
        // Fetch desde TiendaNube
        const tnResponse = await axios.get(
          `https://api.tiendanube.com/v1/${storeId}/orders/${order.tn_order_id}`,
          {
            headers: {
              'Authentication': `bearer ${process.env.TIENDANUBE_ACCESS_TOKEN}`,
              'User-Agent': 'PetLove CRM (nahuelito95@gmail.com)'
            }
          }
        );

        const pedido = tnResponse.data;

        // Guardar productos (con lógica de comparación y borrado)
        await guardarProductos(order.order_number, pedido.products || []);

        console.log(`✅ Resync ${order.order_number}: ${(pedido.products || []).length} productos`);
        resynced++;
        ...
        console.error(`❌ Error resync ${order.order_number}:`, err.message);
        errors.push({ order: order.order_number, error: err.message });
      }
    }

    res.json({
      message: `Resync completado`,
      total: orders.length,
      resynced,
    ...
    console.error('❌ /temp-fix error:', error.message);
    res.status(500).json({ error: error.message });
  }
});
